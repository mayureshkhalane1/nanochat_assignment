...
## Assignment: commands and run logs

This fork was used to complete the coursework assignment. The exact commands and
the code/checkpoints we produced are recorded here and in [`runs/`](runs/) so each
task is reproducible. Effective hyper-parameters match the `user_config` written
into each checkpoint's `meta_*.json` (see [`checkpoints/`](checkpoints/)).
